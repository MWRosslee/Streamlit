...
# CSS Style
with open('/Users/mwrosslee/PycharmProjects/AnalyticsDashboard-with-Excel-file/style.css') as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html = True)

#load excel file
df=pd.read_excel('/Users/mwrosslee/PycharmProjects/AnalyticsDashboard-with-Excel-file/data.xlsx', sheet_name='Sheet1')
...
# Assuming you are working with a column named 'Investment' in df_selection DataFrame
mode_series = df_selection['Investment'].mode()

if len(mode_series) == 1:
    investment_mode = float(mode_series.iloc[0])
else:
    investment_mode = None  # Handle multiple modes or no mode
...
#1. simple bar graph
    # Assuming the intention is to sum the investments per business type
investment_by_businessType = df_selection.groupby(by=["BusinessType"]).sum()[["Investment"]].sort_values(by="Investment")
...
#2. simple line graph------------------
investment_by_state = df_selection.groupby(by=["State"]).sum()[["Investment"]]
fig_state = px.line(
    investment_by_state,
